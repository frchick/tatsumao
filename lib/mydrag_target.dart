...
/// Signature for determining information about the acceptance by a [MyDragTarget].
///
/// Used by [MyDragTarget.onAcceptWithDetails].
typedef MyDragTargetAcceptWithDetails<T> = void Function(MyDragTargetDetails<T> details);
...
typedef MyDragTargetBuilder<T> = Widget Function(BuildContext context, List<T?> candidateData, List<dynamic> rejectedData);

/// Signature for when a [MyDraggable] is dragged across the screen.
///
/// Used by [MyDraggable.onDragUpdate].
typedef MyDragUpdateCallback = void Function(DragUpdateDetails details);

/// Signature for when a [MyDraggable] is dropped without being accepted by a [MyDragTarget].
///
/// Used by [MyDraggable.onDraggableCanceled].
typedef MyDraggableCanceledCallback<T> = void Function(Velocity velocity, Offset offset, T data);

/// Signature for when the draggable is dropped.
///
/// The velocity and offset at which the pointer was moving when the draggable
/// was dropped is available in the [MyDraggableDetails]. Also included in the
/// `details` is whether the draggable's [MyDragTarget] accepted it.
///
/// Used by [MyDraggable.onDragEnd].
typedef MyDragEndCallback<T> = void Function(MyDraggableDetails<T> details);
...
/// Signature for when a [MyDraggable] moves within a [MyDragTarget].
///
/// Used by [MyDragTarget.onMove].
typedef MyDragTargetMove<T> = void Function(MyDragTargetDetails<T> details);

/// Signature for the strategy that determines the drag start point of a [MyDraggable].
///
/// Used by [MyDraggable.dragAnchorStrategy].
///
...
typedef MyDragAnchorStrategy = Offset Function(MyDraggable<Object> draggable, BuildContext context, Offset position);
...
///
/// This is the default [MyDragAnchorStrategy] and replaces [MyDragAnchor.child].
///
/// See also:
///
///  * [MyDragAnchorStrategy], the typedef that this function implements.
///  * [MyDraggable.dragAnchorStrategy], for which this is a built-in value.
...
///
/// This replaces [MyDragAnchor.pointer], which has been deprecated.
///
/// See also:
///
///  * [MyDragAnchorStrategy], the typedef that this function implements.
///  * [MyDraggable.dragAnchorStrategy], for which this is a built-in value.
Offset pointerDragAnchorStrategy(MyDraggable<Object> draggable, BuildContext context, Offset position) {
  return Offset.zero;
}

/// Where the [MyDraggable] should be anchored during a drag.
///
/// This has been replaced by the more configurable [MyDragAnchorStrategy].
@Deprecated(
...
///
/// {@tool dartpad}
/// The following example has a [MyDraggable] widget along with a [MyDragTarget]
...
class MyDraggable<T extends Object> extends StatefulWidget {
  ...
  const MyDraggable({
    ...
    @Deprecated(
      'Use dragAnchorStrategy instead. '
      'Replace "dragAnchor: MyDragAnchor.child" with "dragAnchorStrategy: childDragAnchorStrategy". '
      'Replace "dragAnchor: MyDragAnchor.pointer" with "dragAnchorStrategy: pointerDragAnchorStrategy". '
      'This feature was deprecated after v2.1.0-10.0.pre.',
    )
  ...
  /// event, see [MyDraggable.affinity].
  final Axis? axis;
  ...
  ///
  /// Defaults to [MyDragAnchor.child].
  @Deprecated(
    'Use dragAnchorStrategy instead. '
    'This feature was deprecated after v2.1.0-10.0.pre.',
  )
  ...
  ///
  /// This property's value is a function that implements [MyDragAnchorStrategy].
  ...
  /// property is set to [MyDragAnchor.child], and [pointerDragAnchorStrategy] if
  /// the [dragAnchor] is set to [MyDragAnchor.pointer].
  ...
  final MyDragUpdateCallback? onDragUpdate;

  /// Called when the draggable is dropped without being accepted by a [MyDragTarget].
  ///
  ...
  final MyDraggableCanceledCallback? onDraggableCanceled;

  /// Called when the draggable is dropped and accepted by a [MyDragTarget].
  ///
  /// This function might be called after this widget has been removed from the
  /// tree. For example, if a drag was in progress when this widget was removed
  ...
  /// dropped is available in the [MyDraggableDetails]. Also included in the
  /// `details` is whether the draggable's [MyDragTarget] accepted it.
  ///
  /// This function will only be called while this widget is still mounted to
  /// the tree (i.e. [State.mounted] is true).
  final MyDragEndCallback? onDragEnd;
...
/// See also:
///
///  * [MyDraggable], similar to the [LongPressDraggable] widget but happens immediately.
///  * [MyDragTarget], a widget that receives data when a [MyDraggable] widget is dropped.
class LongPressDraggable<T extends Object> extends MyDraggable<T> {
...
class _MyDraggableState<T extends Object> extends State<MyDraggable<T>> {
  ...
  _MyDragAvatar<T>? _startDrag(Offset position) {
    if (widget.maxSimultaneousDrags != null && _activeCount >= widget.maxSimultaneousDrags!) {
      return null;
    }
    ...
    final _MyDragAvatar<T> avatar = _MyDragAvatar<T>(
      ...
      onDragEnd: (Velocity velocity, Offset offset, bool wasAccepted) {
        ...
        if (mounted && widget.onDragEnd != null) {
          widget.onDragEnd!(MyDraggableDetails(
        ...
        if (!wasAccepted && widget.onDraggableCanceled != null) {
          widget.onDraggableCanceled!(velocity, offset, widget.data);
        }
      },
    );
...
/// when the draggable event occurred, and whether its [MyDragTarget] accepted it.
///
/// Also, this is the details object for callbacks that use [MyDragEndCallback].
class MyDraggableDetails<T> {
  /// Creates details for a [MyDraggableDetails].
  ///
  ...
  /// Determines whether the [MyDragTarget] accepted this draggable.
...
/// Represents the details when a pointer event occurred on the [MyDragTarget].
class MyDragTargetDetails<T> {
  /// Creates details for a [MyDragTarget] callback.
  ///
  /// The [offset] must not be null.
  MyDragTargetDetails({required this.data, required this.offset}) : assert(offset != null);

  /// The data that was dropped onto this [MyDragTarget].
  final T data;
...
class MyDragTarget<T extends Object> extends StatefulWidget {
  ...
  /// [MyDragTargetDetails].
  final MyDragTargetAcceptWithDetails<T>? onAcceptWithDetails;

  /// Called when a given piece of data being dragged over this target leaves
  /// the target.
  final MyDragTargetLeave<T>? onLeave;

  /// Called when a [MyDraggable] moves within this [MyDragTarget].
  ///
...
List<T?> _mapAvatarsToData<T extends Object>(List<_MyDragAvatar<Object>> avatars) {
  return avatars.map<T?>((_MyDragAvatar<Object> avatar) => avatar.data as T?).toList();
}

class _MyDragTargetState<T extends Object> extends State<MyDragTarget<T>> {
  final List<_MyDragAvatar<Object>> _candidateAvatars = <_MyDragAvatar<Object>>[];
  final List<_MyDragAvatar<Object>> _rejectedAvatars = <_MyDragAvatar<Object>>[];
  ...
  bool didEnter(_MyDragAvatar<Object> avatar) {
  ...
  void didLeave(_MyDragAvatar<Object> avatar) {
  ...
  void didDrop(_MyDragAvatar<Object> avatar) {
    assert(_candidateAvatars.contains(avatar));
    if (!mounted) {
      return;
    }
    ...
    widget.onAcceptWithDetails?.call(MyDragTargetDetails<T>(data: avatar.data! as T, offset: avatar._lastOffset!));
  }

  void didMove(_MyDragAvatar<Object> avatar) {
    if (!mounted) {
      return;
    }
    widget.onMove?.call(MyDragTargetDetails<T>(data: avatar.data! as T, offset: avatar._lastOffset!));
  }
...
class _MyDragAvatar<T extends Object> extends Drag {
  _MyDragAvatar({
  ...
  Iterable<_MyDragTargetState<Object>> _getDragTargets(Iterable<HitTestEntry> path) {
    ...
    final List<_MyDragTargetState<Object>> targets = <_MyDragTargetState<Object>>[];
    for (final HitTestEntry entry in path) {
      final HitTestTarget target = entry.target;
      if (target is RenderMetaData) {
        final dynamic metaData = target.metaData;
        if (metaData is _MyDragTargetState && metaData.isExpectedDataType(data, T)) {
